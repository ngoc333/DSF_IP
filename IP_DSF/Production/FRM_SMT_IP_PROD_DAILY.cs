...
namespace IP
{
    public partial class FRM_SMT_IP_PROD_DAILY : Form
    {
        public FRM_SMT_IP_PROD_DAILY()
        {
            InitializeComponent();
        }
        ...
        void DWMYClick(string ButtonCap, string ButtonCD)
        {
            ...
            switch (ButtonCD)
            {
                ...
                case "D":
                    this.Close();
                    Form fc = Application.OpenForms["FRM_SMT_IP_PROD_DAILY"];
                    if (fc != null)
                        fc.Show();
                    else
                    {
                        FRM_SMT_IP_PROD_DAILY f = new FRM_SMT_IP_PROD_DAILY();
                        f.Show();
                    }
                    break;
                case "M":
        ...
        private void formatband()
        {
            try
            {
                ...
                dtsource = db.SEL_IP_PROD_DAILY("H", "", "");
        ...
        private void BindingData(string arg_op)
        {
            try
            {
                ...
                grdView.Refresh();
                DataTable dtsource = null;
                dtsource = db.SEL_IP_PROD_DAILY("Q", arg_op, dtpDate.DateTime.ToString("yyyyMMdd"));
                //formatband();
                DataTable dt = null;
                if (dtsource == null)
                {
