...
namespace OS_DSF
{
    public partial class FRM_SMT_IP_OSD_YEAR : Form
    {
        public FRM_SMT_IP_OSD_YEAR()
        {
            InitializeComponent();
        }
        ...
        private void FRM_SMT_IP_OSD_YEAR_Load(object sender, EventArgs e)
        {            
            timer1.Enabled = true;
            timer1.Start();
        ...
        void DWMYClick(string ButtonCap, string ButtonCD)
        {
            ...
            switch (ButtonCD)
            {
                ...
                case "M":
                    this.Close();
                    Form fc = Application.OpenForms["FRM_SMT_IP_OSD_MONTH"];
                    if (fc != null)
                        fc.Show();
                    else
                    {
                        FRM_SMT_IP_OSD_MONTH f = new FRM_SMT_IP_OSD_MONTH();
                        f.Show();
                    }
                    break;
                case "Y":
                    this.Close();
                    Form fc1 = Application.OpenForms["FRM_SMT_IP_OSD_YEAR"];
                    if (fc1 != null)
                        fc1.Show();
                    else
                    {
        ...
        private void BindingData(string arg_op)
        {
            grdView.Refresh();
            DataTable dtsource = null;
            dtsource = db.SEL_OS_OSD_YEAR("Q", arg_op);
            //formatband();
        ...
        private void bindingdatachart(string arg_op)
        {
            ...
            {
                DataTable dt = null;
                dt = db.SEL_OS_OSD_YEAR("C", arg_op);
        ...
        private void FRM_SMT_IP_OSD_YEAR_VisibleChanged(object sender, EventArgs e)
        {
            try
            {
                if (this.Visible)
