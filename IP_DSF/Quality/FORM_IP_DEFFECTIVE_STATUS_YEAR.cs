...
namespace IP
{
    public partial class FORM_IP_DEFFECTIVE_STATUS_YEAR: Form
    {
        #region Declare
        int iNumRow = 0;
       ...
       // int _time = 0;
        ...
        #region Creation
        public FORM_IP_DEFFECTIVE_STATUS_YEAR(int arg_idx = 0)
        {
            InitializeComponent();
            idx_form = arg_idx;
        }
               ...
               // dt = null;
                ...
                if (dt != null && dt.Rows.Count > 1)
                {
                    ...
                    if (dt_Daily_Report != null && dt_Daily_Report.Rows.Count > 1)
                    {
                        ...
                        axfpSpread.SetText(5, dt.Rows.Count + 2, (Convert.ToDouble(dt.Rows[0][8].ToString())  == 0 ? "0" : (total[0] / Convert.ToDouble(dt.Rows[0][8].ToString()) * 100).ToString("###,##0.00")));
                        axfpSpread.SetText(6, dt.Rows.Count + 2, (Convert.ToDouble(dt.Rows[0][9].ToString()) == 0 ? "0" : (total[1] / Convert.ToDouble(dt.Rows[0][9].ToString()) * 100).ToString("###,##0.00")));
                        axfpSpread.SetText(7, dt.Rows.Count + 2, (Convert.ToDouble(dt.Rows[0][10].ToString())  == 0 ? "0" : (total[2] / Convert.ToDouble(dt.Rows[0][10].ToString()) * 100).ToString("###,##0.00")));
                        axfpSpread.SetText(8, dt.Rows.Count + 2, (Convert.ToDouble(dt.Rows[0][11].ToString())  == 0 ? "0" : (total[3] / Convert.ToDouble(dt.Rows[0][11].ToString()) * 100).ToString("###,##0.00")));
                        axfpSpread.SetText(9, dt.Rows.Count + 2, (Convert.ToDouble(dt.Rows[0][12].ToString())  == 0 ? "0" : (total[4] / Convert.ToDouble(dt.Rows[0][12].ToString()) * 100).ToString("###,##0.00")));
                        axfpSpread.SetText(10, dt.Rows.Count + 2, (Convert.ToDouble(dt.Rows[0][13].ToString())  == 0 ? "0" : (total[5] / Convert.ToDouble(dt.Rows[0][13].ToString()) * 100).ToString("###,##0.00")));
                        axfpSpread.SetText(11, dt.Rows.Count + 2, (Convert.ToDouble(dt.Rows[0][14].ToString())  == 0 ? "0" : (total[6] / Convert.ToDouble(dt.Rows[0][14].ToString()) * 100).ToString("###,##0.00")));

                        axfpSpread.SetText(12, dt.Rows.Count + 2, (Convert.ToDouble(dt.Rows[0][15].ToString()) == 0 ? "0" : (total[7] / Convert.ToDouble(dt.Rows[0][15].ToString()) * 100).ToString("###,##0.00")));
                        axfpSpread.SetText(13, dt.Rows.Count + 2, (Convert.ToDouble(dt.Rows[0][16].ToString()) == 0 ? "0" : (total[8] / Convert.ToDouble(dt.Rows[0][16].ToString()) * 100).ToString("###,##0.00")));
                        axfpSpread.SetText(14, dt.Rows.Count + 2, (Convert.ToDouble(dt.Rows[0][17].ToString()) == 0 ? "0" : (total[9] / Convert.ToDouble(dt.Rows[0][17].ToString()) * 100).ToString("###,##0.00")));
                        axfpSpread.SetText(15, dt.Rows.Count + 2, (Convert.ToDouble(dt.Rows[0][18].ToString()) == 0 ? "0" : (total[10] / Convert.ToDouble(dt.Rows[0][18].ToString()) * 100).ToString("###,##0.00")));
                        axfpSpread.SetText(16, dt.Rows.Count + 2, (Convert.ToDouble(dt.Rows[0][19].ToString()) == 0 ? "0" : (total[11] / Convert.ToDouble(dt.Rows[0][19].ToString()) * 100).ToString("###,##0.00")));                       
        ...
        private DataTable select_deffect_monitor(string sYear)
        {
            ...
            MyOraDB.Process_Name = "MES.PKG_SMT_B2_IP.SEL_IP_DEFECT_MONITOR_YEAR";
        ...
        private DataTable select_chart_1(string arg_year)
        {
            ...
            COM.OraDB MyOraDB = new COM.OraDB();
            MyOraDB.ReDim_Parameter(2);
            MyOraDB.Process_Name = "MES.PKG_SMT_B2_IP.SELECT_CHART_1_YEAR_V2";
        ...
        private DataTable select_chart_2(string arg_year)
        {
            ...
            COM.OraDB MyOraDB = new COM.OraDB();
            MyOraDB.ReDim_Parameter(2);
            MyOraDB.Process_Name = "MES.PKG_SMT_B2_IP.SELECT_CHART_2_YEAR_V2";
        ...
        private void FORM_DMP_DEFFECTIVE_STATUS_YEAR_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                this.button1.BackgroundImage = global::IP.Properties.Resources.Back_Icon;
